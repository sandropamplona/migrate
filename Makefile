...
#Seguinte linha substitui o valor original ../../cli/build/
FOLDER_TAG ?= /output/
NOME_BINARIO ?= nome.arquivo.binario
...
build-cli-dsin: clean
	-mkdir ./cli/build
	cd ./cmd/migrate && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -o $(FOLDER_TAG)$(NOME_BINARIO)x64 -ldflags='-X main.Version=$(VERSION) -extldflags "-static"' -tags '$(DATABASE) $(SOURCE)' .
	cd ./cmd/migrate && CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -a -o $(FOLDER_TAG)$(NOME_BINARIO)x64.exe -ldflags='-X main.Version=$(VERSION) -extldflags "-static"' -tags '$(DATABASE) $(SOURCE)' .
