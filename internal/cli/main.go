...
import (
	...
	"syscall"
	"time"
	//Caso os arquivos gerados pelo go-bindata não devam ser versionados,as
        //pastas migrationsSGITDados e migrationsSGITImagens, deverão ficar dento
        //da pasta informado na variável de ambiente GOROOT.
        //Via linha de comando, digitar go env, para ver o valor de GOROOT.
        //Nesse caso, as duas seguintes linhas, deverão ser descomentada, e as
        //duas subseguintes linhas deverão ser comentadas.
	//migrationDSIN_SGITDados "migrationsSGITDados"
	//migrationDSIN_SGITImagens "migrationsSGITImagens"
	migrationDSIN_SGITDados "github.com/golang-migrate/migrate/v4/internal/cli/migrationsSGITDados"
        migrationDSIN_SGITImagens "github.com/golang-migrate/migrate/v4/internal/cli/migrationsSGITImagens"
	...
	v "github.com/golang-migrate/migrate/v4/internal/cli/UtilsDSIN"
)
...
const (
	...
	createUsage       = `create [-ext E] [-dir D] [-seq] [-digits N] [-format] [-tz] NAME
	   ...
	   Use -format option to specify a Go time format string. Note: migrations with the same time cause "duplicate migration version" error.
           Use -tz option to specify the timezone that will be used when generating non-sequential migrations (defaults: America/Sao_Paulo).
`
...
Database drivers: `+strings.Join(database.List(), ", ")+"\n", createUsage, gotoUsage, upUsage, downUsage, dropUsage, forceUsage)
	...
	var bancoDSIN string
	var er error
	var s *bindata.AssetSource

	if flag.Arg(0) != "create" {
		bancoDSIN, er = v.IdentificadorDB(*databasePtr)

		fmt.Println("----------------------")
		fmt.Println("Banco de dados:", bancoDSIN)
		fmt.Println("----------------------")
		...
		if bancoDSIN == "SGITDADOS" {
			s = bindata.Resource(migrationDSIN_SGITDados.AssetNames(),
				func(name string) ([]byte, error) {
					return migrationDSIN_SGITDados.Asset(name)
				})
		} else if bancoDSIN == "SGITIMAGENS" {
			s = bindata.Resource(migrationDSIN_SGITImagens.AssetNames(),
				func(name string) ([]byte, error) {
					return migrationDSIN_SGITImagens.Asset(name)
				})
		}
	}
	...
	if bancoDSIN != "" {
		...
		if err != nil {
			fmt.Println("Ocorreu um erro na tentativa de criar uma instancia do go-bindata:", err)
		}

		if d == nil {
			fmt.Println("go-bindata, vazio")
		}

		migrater, migraterErr = migrate.NewWithSourceInstance("go-bindata", d, *databasePtr)
	} else {
	...
	case "create":
		...
		createFlagSet.IntVar(&seqDigits, "digits", seqDigits, "The number of digits to use in sequences (default: 7)")
